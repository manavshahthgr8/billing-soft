document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("markBilled").addEventListener("click", async () => {
        let selectedTransactionIds = [];
        document.querySelectorAll(".txn-checkbox:checked").forEach(checkbox => {
            selectedTransactionIds.push(checkbox.value); // ✅ Now it will capture `transaction_id`
        });
    
        //console.log("✅ Selected Transaction IDs:", selectedTransactionIds); // Debugging Log
        ...
        if (selectedTransactionIds.length === 0) {
         ...
         // Confirmation alert
        let confirmAction = confirm("By pressing OK, selected transactions will be marked as billed.");
        if (!confirmAction) return; // If user clicks "Cancel", stop execution
    ...
    async function generateAndDownloadPDF() {
        ...
        const printTableHeaders = (yPos) => {
            ...
            return yPos + 6;  // Return next row position
        };
        ...
        document.querySelectorAll(".txn-checkbox:checked").forEach(checkbox => {
            ...
            selectedTransactions.push({
                ...
                sno: row[1].textContent.trim(),  // ✅ Extracted S.No
                date: row[2].textContent.trim(), // ✅ Adjusted index
                party: party,
                city: city,
                type: txnType,
                item: row[6].textContent.trim(),  // ✅ Adjusted index
                qty: row[7].textContent.trim(),   // ✅ Adjusted index
                bhav: row[8].textContent.trim(),  // ✅ Adjusted index
                rate: row[9].textContent.trim(),  // ✅ Adjusted index
        ...
        let bottomMargin = 1;  // **Reduced for more rows**
        let rowHeight = 6;  // **Reduced to fit more data**
        let pageNumber = 1;
        ...
        selectedTransactions.forEach((txn, index) => {
            if (rowY + rowHeight > pageHeight - bottomMargin) {
                ...
                doc.text(
                    `${firmName} | FY ${fyDisplay} | ${customerName} Invoice `,
                    10,
                    pageHeight - 3,
                ...
                doc.setFont("times", "normal");
                doc.text(
                    `Continued on next page | Page ${pageNumber}`,
                    170,
                    pageHeight - 3,
                    { align: "center" }
                );
            ...
            if (index % 2 === 0) {
                ...
                doc.rect(5, rowY - 4, 200, rowHeight, "F");  // Adjusted height
            }
        ...
        // **📏 Add Border Around Table (on First Page only)**
        if (firstPage) {
            doc.rect(5, startY - 4, 200, rowY - startY + 3);
        }
    
        // **📌 Footer: Total & Closing Message**
        ...
        doc.text("Thank you for being a valuable customer.", 105, rowY, { align: "center" });
        ...
        const firmFirstName = firmName.split(" ")[0];

        // Generate filename
        const fileName = `${customerFirstName}Invoice_${firmFirstName}FY${financialYear}.pdf`;
    ...
    document.getElementById("preview").addEventListener("click", async () => {
        ...
        // **📄 Open PDF in New Tab**
        // Open the print dialog instantly
        //doc.autoPrint();
